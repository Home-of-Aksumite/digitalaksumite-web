/**
 * Footer Component
 * Site footer with navy background and gold accents
 */
...
export function Footer({ siteSettings }: FooterProps) {
  ...
  const socialLinks = [
    ...(siteSettings?.linkedin
      ? [{ href: siteSettings.linkedin, label: 'LinkedIn', icon: LinkedInIcon }]
      : []),
    ...(siteSettings?.twitter
      ? [{ href: siteSettings.twitter, label: 'Twitter', icon: TwitterIcon }]
      : []),
    ...(siteSettings?.github
      ? [{ href: siteSettings.github, label: 'GitHub', icon: GitHubIcon }]
      : []),
    ...(siteSettings?.instagram
      ? [{ href: siteSettings.instagram, label: 'Instagram', icon: InstagramIcon }]
      : []),
    ...(siteSettings?.facebook
      ? [{ href: siteSettings.facebook, label: 'Facebook', icon: FacebookIcon }]
      : []),
  ];
              ...
              <div className="mt-6 flex gap-4">
                {socialLinks.map((social) => (
                  <a
                    ...
                    className="text-[#E5E7EB]/60 transition-colors hover:text-[#C9A227]"
              ...
              <ul className="mt-4 space-y-3">
                {footerLinks.company.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-[#E5E7EB]/80 transition-colors hover:text-white"
                    >
                      {link.label}
                    </Link>
                  </li>
